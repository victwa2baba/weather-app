...
async function getWeatherData(location, result) {
  try {
    const data = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location}?unitGroup=us&include=days%2Calerts%2Chours&key=${apiKey1}&contentType=json`,
      { mode: "cors" },
    );
...
async function header() {
  ...
  container.setAttribute(
    "class",
    "align-items-center justify-content-center input-group",
  );
...
async function displayContent() {
  ...
  neededData.days.forEach((element, index) => {
    if (index < 5) {
      ...
      weekdaysInner.setAttribute(
        "class",
        "weekdays-inner gap-3 align-grid-center",
      );
